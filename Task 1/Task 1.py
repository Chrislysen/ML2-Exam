...
IMG_DIR = os.path.join(SCRIPT_DIR, 'images')   # Assumes images folder is next to script
LABELS_FILE = os.path.join(SCRIPT_DIR, 'labels.csv')  # Assumes labels.csv is next to script

IMG_SIZE = (32, 32)
BATCH_SIZE = 64
EPOCHS = 40  # training cycles
LEARNING_RATE = 1e-3  # Learning-rate
SEED = 44  # choose the seed, in order to reproduce the same results
...
print("LABELS_FILE:", LABELS_FILE, "exists:", os.path.exists(LABELS_FILE))
...
print(f"Total samples loaded: {X.shape[0]}")  # Should be 1800
...
# 1. Compute metrics (Accuracy, Precision, Recall, F1)
...
print(
    classification_report(
        y_true, y_pred, target_names=["Circle", "Square", "Triangle"]
    )
)
...
# 3. Display 6 random test images with predicted and true labels
